	...
	while( true ) 
    {
		...
		for(int i = UpY; i < DownY; i++)
		{
			for(int j = UpX; j < DownX; j++)
			{
				...
				avgDist = BackgroundFrame.at<unsigned short>(i, j)-4;
		...
		else if((resetCounter > 3) && (OutX.size() > 0))
		{
			resetCounter = 0;
			averageX = 0;
			averageY = 0;
...
void fingerDetection(cv::Mat binaryImg, cv::Mat colorImg, vector<int> OldOutX, vector<int> OldOutY)
{
	...
	for( int i = 0; i< contours.size(); i++ )
    {
		...
		if((area > 30) && (area < 70) && (circularity > 0.2) && (circularity < 0.8))
		{
			float furthestReach;
			///////////////////////////////////////////////////////
			//Draw stuff

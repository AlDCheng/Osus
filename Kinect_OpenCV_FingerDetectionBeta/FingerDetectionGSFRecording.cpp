...
//	float response = SVMFinger.predict(sampleMat);
	...
	while( true ) 
    {
		...
		resize(falseColorsMap, falseColorsMap, Size(320,240));
		...
		for(int i = UpY; i < DownY; i++)
		{
			for(int j = UpX; j < DownX; j++)
			{
				...
				if((depthVal > (avgDist-25)) && (depthVal <= (avgDist-5)))
				{
		...
		resize(mImageThres, mImageThres, Size(320,240));
		...
		resize(canvas, canvas, Size(683,384));

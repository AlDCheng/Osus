...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while( true )
	{
		...
		putText(cImageBGR, DownXYCali, cvPoint(DownXCali, DownYCali-20), 
			FONT_HERSHEY_COMPLEX_SMALL, 1, cvScalar(255, 255, 0));
		rectangle(cImageBGR, cvPoint(UpXCali, UpYCali), cvPoint(DownXCali, DownYCali), cvScalar(255,255,0), 1);
		//---------------------------------------------------------------
		//---------------------------------------------------------------
		cv::imshow( "Depth Image (Init-Binary)", mImageThres );
		//---------------------------------------------------------------
		Calibration( mImageThres, cImageBGR, &centerB[0][0], &centerB[1][0], &blockDL, &blockDW );
		...
		//cout << "CenterA: (" << centerA[0][0] << "," << centerA[1][0] << ")    " <<
		//	"CenterB: (" << centerB[0][0] << "," << centerB[1][0] << ")" << endl;

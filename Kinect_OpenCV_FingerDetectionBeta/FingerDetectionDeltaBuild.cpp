...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while( true )
	{
		...
		for(int i = UpY; i < DownY; i++)
		{
			...
			{
				int pixelVal = frameRGB[1].data[frameRGB[1].step[0]*i + frameRGB[1].step[1]*j];
				if((100 > pixelVal) && (pixelVal > 60))
				{
					frameColorThres.data[frameColorThres.step[0]*i + frameColorThres.step[1]*j] = 255;
				}
				else
				{
					frameColorThres.data[frameColorThres.step[0]*i + frameColorThres.step[1]*j] = 0;
				}
			}
		}
		...
		for(int i = UpY; i < DownY; i++)
		{
			for(int j = UpX; j < DownX; j++)
			{
				...
				if((depthVal <= avgDist-5) && (depthVal > 100))
				{
